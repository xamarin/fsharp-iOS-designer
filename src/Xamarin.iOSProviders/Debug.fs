    ...
    let prettyPrint signatureOnly ignoreOutput maxDepth maxWidth (t: ProvidedTypeDefinition) = 
        ...
        let printMember (memberInfo: MemberInfo) =        
            ...
            let getMethodBody (m: ProvidedMethod) = 
                seq { if not m.IsStatic then
                        yield if t.IsErased then (ProvidedTypeDefinition.EraseType m.DeclaringType)
                              else m.DeclaringType
                      for param in m.GetParameters() do
                        yield if t.IsErased then (ProvidedTypeDefinition.EraseType param.ParameterType)
            ...
            let getConstructorBody (c: ProvidedConstructor) = 
                if c.IsImplicitCtor then Expr.Value(()) else
                    seq { for param in c.GetParameters() do
                            yield if t.IsErased then (ProvidedTypeDefinition.EraseType param.ParameterType)
                                  else param.ParameterType }
                    |> Seq.map (fun typ -> Expr.Value(null, typ))
                    |> Array.ofSeq
